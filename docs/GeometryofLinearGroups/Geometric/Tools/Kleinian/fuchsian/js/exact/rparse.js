...
function tokenize(s) {
    ...
    while (i < s.length) {
        ...
        if (/[a-zA-Z]/.test(ch)) {                 // identifier (a constant name, or √/cos/π ⇒ thrown later)
            let j = i;
...
class Parser {
    constructor(toks, scope = {}) { this.toks = toks; this.pos = 0; this.scope = scope; }
    ...
    term() {
        ...
        for (;;) {
            ...
            } else if (t === '(' || (t !== undefined && /^[0-9.a-zA-Z]/.test(t))) {
    ...
    primary() {
        ...
        if (t !== undefined && /^[a-zA-Z]/.test(t)) {            // a named constant from the scope
            this.next();
            const v = this.scope[t];
            if (v instanceof BigRational) return v;
            throw new Error(`"${t}" is not a rational constant (entries must lie in ℚ)`);
        }
 ...
 * `scope` maps constant names → BigRational (e.g. {t: 2}); anything else (√, cos, π,
 * or an undefined name) throws — which callers use to detect non-ℚ entries.
 */
export function parseRational(input, scope = {}) {
