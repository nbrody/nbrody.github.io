/**
 * Mirror rendering: multi-bounce reflections that travel along
...
export const mirrorFragmentShader = `
    ...
    vec3 faceColor(int idx) {
        return idx >= 0 ? u_faceColor[idx] : vec3(0.02);
    }
    ...
    float edgeFactor(vec3 p, int idx) {
        ...
        return 1.0 - smoothstep(0.0, max(u_edgeLightWidth, 1e-6), abs(sdWall(p, u_faces[idx]) - d2));
    }
