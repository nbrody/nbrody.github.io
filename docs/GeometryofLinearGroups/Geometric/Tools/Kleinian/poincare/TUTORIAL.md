...
Where it lives: this is the `?tutorial=true` mode of the live tool,
`docs/GeometryofLinearGroups/Geometric/Tools/Kleinian/poincare`. The MathFest deck
embeds its own frozen copy, `docs/Talks/MathFest/poincareTutorial` (taken from an
earlier poincare2 snapshot, free to diverge for talk purposes), so changes here do
not reach the talk unless copied over.
...
`poincare:refreshed` window event after every refresh. (Since the Sept 2026
rewrite `main.js` is the orchestrator over `app.js`, `scene.js`, `overlays.js`,
`inspector.js`, … and the domain is computed in a Web Worker, so a refresh is
asynchronous: wait for `poincare:refreshed`, as tutorial.js does.)
