...
// Populate sphere preset dropdown with optgroup categories
...
Object.entries(presetCategories).forEach(([catKey, catLabel]) => {
    if (!grouped[catKey] || grouped[catKey].length === 0) return;
    const optgroup = document.createElement('optgroup');
    optgroup.label = catLabel;
    grouped[catKey].forEach(opt => {
...
// Add custom option outside groups
if (grouped['custom']) {
    grouped['custom'].forEach(opt => {
        const el = document.createElement('option');
        el.value = opt.value;
        el.textContent = opt.name;
...
// Apply preset-specific rendering parameters
function applyPresetParams(params) {
    if (params.maxIterations !== undefined) {
        uniforms.maxIterations.value = params.maxIterations;
        baseMaxIterations = params.maxIterations;
        controls_ui.maxIterations.value = params.maxIterations;
    }
    updateDisplays();
}
...
function loadPreset(presetName) {
    ...
    const preset = spherePresets[presetName];
    currentSpheres = getPresetSpheres(presetName);
    currentPreset = presetName;
    // Auto-switch to flexible mode for non-default presets
    // (fast mode hardcodes default sphere positions in the shader)
    if (presetName !== 'default' && currentPerformanceMode === 'fast') {
        document.getElementById('performanceMode').value = 'flexible';
        switchPerformanceMode('flexible');
    }
    // Apply preset-specific rendering params
    if (preset && preset.params) {
        applyPresetParams(preset.params);
    }
    // Update description
    if (presetDesc && preset) {
