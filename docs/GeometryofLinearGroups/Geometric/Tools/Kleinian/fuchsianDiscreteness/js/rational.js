 ...
 * (Ported from Algebraic/Tools/sRingClosure/js/rational.js, exposed as an ES module.)
 */
...
export class BigRational {
    ...
    static fromInt(n) { return new BigRational(BigInt(n), 1n); }
    static fromFraction(num, den) { return new BigRational(BigInt(num), BigInt(den)); }
    ...
    add(other) {
        ...
        return new BigRational(this.num * other.den + other.num * this.den, this.den * other.den);
    }
    sub(other) {
        if (!(other instanceof BigRational)) other = BigRational.fromInt(other);
        return new BigRational(this.num * other.den - other.num * this.den, this.den * other.den);
...
// Convenience constructor: Q(3) = 3, Q(1,2) = 1/2.
export const Q = (n, d) => d !== undefined ? new BigRational(BigInt(n), BigInt(d)) : BigRational.fromInt(n);
