 ...
 *
 * The domain is convex (intersection of half-spaces with the bounding ball)
 * and contains the basepoint, so every ray from the basepoint crosses the
 * boundary exactly once — we project the vertices of a subdivided icosahedron
 * radially onto the boundary by bisection. This yields a watertight indexed
 * mesh. Unbounded domains are clipped by the bounding sphere.
 */

import * as THREE from 'three';
import { wallSD, RBOUND } from './math.js';
...
function projectToBoundary(origin, dir, walls) {
    ...
    let lo = 0, hi = 0.25;
    while (domainSDF(at(hi), walls) < 0 && hi < 3 * RBOUND) {
        lo = hi;
        hi *= 1.6;
    }
    for (let i = 0; i < 48; i++) {
...
export async function exportDomainAs3MF(domain, opts = {}) {
    ...
    downloadBlob(blob, 'space_group_domain.3mf');
