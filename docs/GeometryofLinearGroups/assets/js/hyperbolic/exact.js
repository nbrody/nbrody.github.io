...
/**
 * Plain-data form of an exact context { field, gens } (structured-clone safe:
 * BigInts become strings). The field keeps its chosen embedding and its
 * complex-conjugation automorphism, if configured.
 */
export function serializeExactContext(ctx) {
    if (!ctx) return null;
    const K = ctx.field;
    const elem = (e) => e.c.map(fracToPair);
    return {
        gen: K.gen,
        minpoly: K.f.map(fracToPair),
        rootIndex: K.rootIndex,
        conjIsIdentity: K.conjIsIdentity,
        conjPowers: K.conjPowers ? K.conjPowers.map(elem) : null,
        gens: ctx.gens.map(M => ({ a: elem(M.a), b: elem(M.b), c: elem(M.c), d: elem(M.d), anti: M.anti }))
    };
}

export function deserializeExactContext(data) {
    if (!data) return null;
    const K = new NumberField(data.minpoly.map(pairToFrac), data.gen);
    K.rootIndex = Math.min(data.rootIndex || 0, K.roots.length - 1);
    const elem = (arr) => K.elem(arr.map(pairToFrac));
    K.conjIsIdentity = !!data.conjIsIdentity;
    K.conjPowers = data.conjPowers ? data.conjPowers.map(elem) : null;
    const gens = data.gens.map(g => new ExactMat(elem(g.a), elem(g.b), elem(g.c), elem(g.d), g.anti));
    return { field: K, gens };
}

// ---------------- Subfields and minimal polynomials ----------------

/** Coefficient vector of an element in the power basis 1, w, …, w^{n−1}. */
function vec(e) {
    const n = e.K.deg, out = [];
    for (let i = 0; i < n; i++) out.push(e.c[i] || F0);
    return out;
}

/**
 * Incremental row echelon basis over Q with provenance: each stored row
 * remembers the combination of inserted vectors it came from, so a vector
 * that reduces to zero yields the linear relation that kills it.
 */
class QSpan {
    constructor(n) { this.n = n; this.rows = []; this.count = 0; }
    /** Reduce v; returns {zero, combo} where combo expresses v in the inputs. */
    reduce(v) {
        let r = v.slice();
        let combo = new Map([[this.count, F1]]);          // v itself, as a future input
        for (const row of this.rows) {
            const c = r[row.pivot];
            if (c.isZero()) continue;
            const f = c.div(row.v[row.pivot]);
            for (let i = 0; i < this.n; i++) r[i] = r[i].sub(row.v[i].mul(f));
            for (const [k, x] of row.combo) combo.set(k, (combo.get(k) || F0).sub(x.mul(f)));
        }
        const pivot = r.findIndex(x => !x.isZero());
        return { r, pivot, combo };
    }
    /** Insert v; returns null if independent, else the relation (Map input→coeff). */
    insert(v) {
        const red = this.reduce(v);
        const idx = this.count++;
        if (red.pivot < 0) return red.combo;
        this.rows.push({ v: red.r, pivot: red.pivot, combo: red.combo, idx });
        return null;
    }
    get dim() { return this.rows.length; }
}

/** Monic minimal polynomial of α over Q, as Frac coefficients (index = degree). */
export function minimalPolynomial(alpha) {
    const K = alpha.K;
    const span = new QSpan(K.deg);
    let p = K.one();
    for (let k = 0; k <= K.deg; k++) {
        const rel = span.insert(vec(p));
        if (rel) {
            // Σ rel[i]·α^i = 0 with rel[k] = 1 (the new vector's own coefficient).
            const coeffs = [];
            for (let i = 0; i <= k; i++) coeffs.push(rel.get(i) || F0);
            const lead = coeffs[k];
            return coeffs.map(c => c.div(lead));
        }
        p = p.mul(alpha);
    }
    throw new Error('minimal polynomial not found (degree exceeds the field degree)');
}

/**
 * The subfield Q(β₁, …, β_r) ⊂ K: its degree, a primitive element (a small
 * integer combination of the β's) and that element's minimal polynomial.
 */
export function subfieldOf(elems) {
    const K = elems[0]?.K;
    if (!K) return { degree: 1, primitive: null, minpoly: [F0, F1] };
    // Q-span closure under multiplication.
    const span = new QSpan(K.deg);
    const basis = [];
    const add = (e) => { if (!span.insert(vec(e))) basis.push(e); };
    add(K.one());
    for (const b of elems) add(b);
    for (let changed = true; changed && basis.length < K.deg;) {
        changed = false;
        const cur = basis.slice();
        for (const x of cur) for (const y of cur) {
            const before = basis.length;
            add(x.mul(y));
            if (basis.length > before) changed = true;
            if (basis.length >= K.deg) break;
        }
    }
    const degree = basis.length;
    // A primitive element: the first small combination of full degree.
    const nz = elems.filter(e => !e.isZero());
    const tries = [];
    for (let c = 1; c <= 6; c++) tries.push(nz.map((_, i) => (i === 0 ? 1 : c ** i)));
    tries.unshift(nz.map(() => 1));
    for (const coeffs of tries) {
        let prim = K.zero();
        nz.forEach((e, i) => { prim = prim.add(e.mul(K.fromFrac(new Frac(BigInt(coeffs[i]))))); });
        if (nz.length === 0) prim = K.one();
        const mp = minimalPolynomial(prim);
        if (mp.length - 1 === degree) return { degree, primitive: prim, minpoly: mp };
    }
    // Fall back to a basis element of top degree (rare).
    for (const b of basis) {
        const mp = minimalPolynomial(b);
        if (mp.length - 1 === degree) return { degree, primitive: b, minpoly: mp };
    }
    return { degree, primitive: null, minpoly: null };
}

/**
 * Coordinates of x in the power basis of the primitive element π of a
 * subfield F (x must lie in F): x = Σ c_i π^i, i < [F:Q].
 */
export function coordinatesIn(x, prim, degree) {
    const K = x.K;
    const span = new QSpan(K.deg);
    let p = K.one();
    for (let i = 0; i < degree; i++) { span.insert(vec(p)); p = p.mul(prim); }
    const rel = span.insert(vec(x));
    if (!rel) throw new Error('element is not in the subfield');
    const lead = rel.get(degree);
    const out = [];
    for (let i = 0; i < degree; i++) out.push((rel.get(i) || F0).div(lead).neg());
    return out;
}

/** Real and complex roots of a rational polynomial (float), for signatures. */
export function rationalPolyRoots(coeffs) {
    return polyComplexRoots(coeffs.map(c => c.toNumber()));
}

/** Pretty polynomial string in variable x from Frac coefficients. */
export function polyString(coeffs, x = 'x') {
    const terms = [];
    for (let i = coeffs.length - 1; i >= 0; i--) {
        ...
        const coef = (abs.p === 1n && abs.q === 1n && mon) ? '' : abs.toString();
        terms.push({ neg, s: coef && mon ? `${coef}${mon}` : (coef || mon) });
    }
    if (!terms.length) return '0';
    return terms.map((t, k) => (k === 0 ? (t.neg ? '−' : '') : (t.neg ? ' − ' : ' + ')) + t.s).join('');
}
