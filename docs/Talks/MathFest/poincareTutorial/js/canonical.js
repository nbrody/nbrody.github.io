/**
 * Canonical Dirichlet domain pipeline for poincare4.
 *
 * Given generators of a subgroup G < PSL(2,C) and a basepoint p (the image of
 * (0,0,1) ∈ UHS under the view matrix), we compute:
 *
...
import {
    ...
    imageOfOriginUHS, uhsToBall, applyMatrixToBall,
    hypDistProxy, bisectorCov, covToGeom, wallSD, isInsideWalls,
    sampleWallPoints, wallContributes, faceViolation, refineOnWall, projectToWall,
    reduceWord, invertWord
} from './math.js';
...
function faceSamples(walls, i, basepoint, maxCount = 12) {
    ...
    for (const s0 of starts) {
        ...
        if (r.v < -1e-7 && r.p.lengthSq() < 1) { out.push(r.p); break; }
    }
    return out;
}
 ...
 * For a Dirichlet wall of g:   s = h·g^{-1} where h ∈ H is the unique element
 *                              with s(face) back inside the domain.
 * Mutates walls, setting wall.pairing = {matrix, word, partner} (or null).
 */
function assignPairings(walls, conePoint, vMat, vInv) {
    const geoms = walls.map(w => w.geom);

    // Find the wall whose defining element equals m (as a PSL element).
    ...
    for (let i = 0; i < walls.length; i++) {
        ...
        // which is the wall of g^{-1}. (An order-2 elliptic pairs a wall with
        // itself: partner === i.) Algebra lives in the basepoint frame; the
        // GEOMETRIC pairing on the displayed (view-transformed) walls is the
        // conjugate V·g^{-1}·V^{-1}.
 ...
 */
function completeMissingWalls(walls, conePoint, q0, vMat, vInv, maxFaces) {
    const geoms = walls.map(w => w.geom);
    const toAdd = new Map();   // pslKey -> {matrix (pure word), word, seed}

    const propose = (matrix, word, seed) => {
        const m = matrix.normalized();
        const key = pslKey(m);
        if (distFromIdentityPSL(m) < 1e-7) return;
    ...
    for (let i = 0; i < walls.length; i++) {
        ...
        for (const w2 of walls) {
            if (distFromIdentityPSL(sAlg.mul(w2.elem.inv()).normalized()) < 1e-6) { havePartner = true; break; }
        }
        if (!havePartner && samples.length > 0) {
            propose(sAlg, sWord, applyMatrixToBall(sGeom, samples[0]));
        }

        // Face samples escaping the domain reveal missing walls g·k, which
        // cut the claimed face near the offending sample.
        for (const p of samples) {
            const y = applyMatrixToBall(sGeom, p);
    ...
    for (const { matrix, word, seed } of toAdd.values()) {
        ...
        const gq = applyMatrixToBall(vMat.mul(matrix).normalized(), q0);
        ...
        const tr = matrix.a.add(matrix.d);
 ...
 */
function edgeCompletion(walls, orbit, conePoint, q0, vMat, maxFaces) {
    const geoms = walls.map(w => w.geom);
    const wallKeys = new Set(walls.map(w => pslKey(w.elem)));
    const toAdd = new Map();   // pslKey -> {matrix, word, seed}
    ...
    for (const e of edges) {
        ...
        // Snap the arc center onto the exact intersection of the labeled pair
        let x = e.mid.clone();
        for (let k = 0; k < 30; k++) {
            x = projectToWall(x, geoms[e.i]);
            x = projectToWall(x, geoms[e.j]);
        }
        const y = applyMatrixToBall(wA.pairing.matrix, x);
        if (y.lengthSq() >= 1) continue;
        ...
        if (!secondExists && toAdd.size === 0) {
            ...
            for (const entry of orbit) {
                if (wallKeys.has(pslKey(entry.matrix))) continue;
                const gq = applyMatrixToBall(vMat.mul(entry.matrix).normalized(), q0);
                const cov = bisectorCov(conePoint, gq);
                if (!cov) continue;
 ...
 * @param {Matrix2x2} viewMat - current view isometry (det 1)
 * @param {number} maxFaces - cap on accepted walls
 * @param {Object} options - { maxDepth, beamWidth, maxOrbit, skipPairings }
 * @returns {{
...
export function computeCanonicalDomain(generators = [], viewMat = Matrix2x2.identity(), maxFaces = 96, options = {}) {
    ...
    for (const cand of candidates) {
        if (walls.length >= maxFaces) break;
        const gq = applyMatrixToBall(viewMat.mul(cand.matrix).normalized(), q0);
        const cov = bisectorCov(conePoint, gq);
    ...
    if (!skipPairings) {
        assignPairings(walls, conePoint, viewMat, vInv);
        let edgePasses = 0;
        for (let iter = 0; iter < 20; iter++) {
            let added = completeMissingWalls(walls, conePoint, q0, viewMat, vInv, maxFaces);
            ...
            if (added === 0 && edgePasses < 6) {
                edgePasses++;
                added = edgeCompletion(walls, orbit, conePoint, q0, viewMat, maxFaces);
            }
            if (added === 0) break;
            pruneWalls(walls, conePoint);
            assignPairings(walls, conePoint, viewMat, vInv);
