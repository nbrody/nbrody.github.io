...
function makeCreature(key, level = LEVEL) {
  const s = SPECIES[key];
  const t = statsAt(creatureBase(key, level), level);
  const creature = {
    key,
    name: creatureName(key, level),
    dom: s.dom,
    sprite: s.sprite,
    level,
    maxhp: t.maxhp,
...
function gainXp(c, amount) {
  ...
  while (c.level < 60 && c.xp >= xpToNext(c.level)) {
    ...
    const t = statsAt(creatureBase(c.key, c.level), c.level);
    c.maxhp = t.maxhp;
    c.stats = { atk: t.atk, def: t.def, gra: t.gra, aeg: t.aeg, spe: t.spe };
    c.hp = frac === 0 ? 0 : Math.max(1, Math.round(t.maxhp * frac));
    const ev = { lv: c.level, learned: [] };
    const newName = creatureName(c.key, c.level);
    if (newName !== c.name) {
      ev.evolved = { from: c.name, to: newName };
      c.name = newName;
    }
...
function chooseAiMove(self, foe) {
  // Prefer useful recovery when hurt, then compare expected damage and support.
  let best = null,
    bestDmg = -1;
  for (const mk of self.moves) {
    if (self.pp && self.pp[mk] <= 0) continue;
    const m = MOVES[mk];
  ...
  const statusMoves = self.moves.filter((mk) => {
    const m = MOVES[mk];
    return (
      MOVES[mk].cat === "status" &&
      (!self.pp || self.pp[mk] > 0) &&
      (m.fx.heal
        ? self.hp < self.maxhp * 0.5
        : m.fx.buff
          ? self.stages[m.fx.buff.stat] < 2
          : !foe.status)
    );
  });
  const recovery = statusMoves.find((mk) => MOVES[mk].fx.heal);
  if (recovery && self.hp < self.maxhp * 0.4 && rng() < 0.7) return recovery;
  if (statusMoves.length && self.hp > self.maxhp * 0.6 && rng() < 0.25) {
    return statusMoves[Math.floor(rng() * statusMoves.length)];
  }
  return best || statusMoves[0] || "struggle";
}
...
function calcDamage(att, def, m, ev) {
  ...
  if (def.guarding) dmg = Math.max(1, Math.floor(dmg * 0.5));
  if (att.resonance) dmg = Math.max(1, Math.floor(dmg * 1.35));
  if (crit > 1) ev.push({ t: "text", s: "A critical strike!" });
  if (e > 1)
    ev.push({ t: "text", s: "The gods favor the blow — it strikes true!" });
  if (e < 1)
    ev.push({ t: "text", s: "The blow glances — the domain resists." });
  return dmg;
}

function actOnce(att, def, moveKey, attSide, ev) {
  const m = MOVES[moveKey];
  if (!m || att.hp <= 0 || def.hp <= 0) return;
  if (moveKey !== "struggle" && att.pp && att.pp[moveKey] <= 0) return;

  // pre-action status checks
  if (att.status === "sleep") {
    if (att.statusTurns <= 0) {
      att.status = null;
      ev.push({ t: "text", s: `<b>${att.name}</b> stirs awake!` });
    } else {
      att.statusTurns--;
      ev.push({ t: "text", s: `<b>${att.name}</b> slumbers on…` });
      return;
    }
  }
  if (att.status === "charm" && rng() < 0.5) {
    ev.push({
      t: "text",
      s: `<b>${att.name}</b> gazes about, hopelessly charmed!`,
    });
    return;
  }

  if (moveKey !== "struggle" && att.pp) att.pp[moveKey]--;
  ...
  if (m.cat === "status") {
    ...
    if (fx.heal) {
      ...
      ev.push({
        t: "text",
        s: `Ambrosial light — <b>${att.name}</b> recovers!`,
      });
    } else if (fx.buff) {
      att.stages[fx.buff.stat] = Math.min(
        2,
        att.stages[fx.buff.stat] + fx.buff.stages,
      );
      const nm = {
        atk: "Attack",
        def: "Defense",
      ...
      if (def.status) {
        ev.push({
          t: "text",
          s: `But <b>${def.name}</b> is already afflicted…`,
        });
      } else if (rng() < fx.chance) {
        applyStatus(def, fx.status, ev);
...
function endOfTurn(c, ev) {
  ...
  if (c.status === "burn" || c.status === "poison") {
    ...
    ev.push({
      t: "text",
      s: `<b>${c.name}</b> suffers from ${c.status === "burn" ? "its burns" : "the venom"}…`,
    });
  }
  if (c.status === "chill" && rng() < 0.25) {
    c.status = null;
    ev.push({ t: "status" });
    ev.push({ t: "text", s: `<b>${c.name}</b> shakes off the chill!` });
  }
  if (c.status === "charm" && rng() < 0.33) {
    c.status = null;
    ev.push({ t: "status" });
    ev.push({ t: "text", s: `<b>${c.name}</b> snaps out of the charm!` });
  }
  c.guarding = false;
}

/* One full round. Returns event log. Mutates creatures. */
function battleEvents(ally, foe) {
  const events = [];
  events.push = function (...items) {
    for (const item of items) {
      if (item.t === "hp" || item.t === "status")
        item.snapshot = { ally: { ...ally }, foe: { ...foe } };
...
function resolveTurn(ally, foe, allyMoveKey) {
  ...
  const foeMoveKey =
    (typeof state !== "undefined" &&
      ...
      state.battle.intent) ||
    chooseAiMove(foe, ally);
  const pa = MOVES[allyMoveKey].prio || 0,
    pf = MOVES[foeMoveKey].prio || 0;
  let order;
  if (pa !== pf) order = pa > pf ? ["ally", "foe"] : ["foe", "ally"];
  else
    order =
      effStat(ally, "spe") >= effStat(foe, "spe")
        ? ["ally", "foe"]
        : ["foe", "ally"];
  ...
  for (const side of order) {
    ...
    if (def.hp <= 0) {
      ev.push({
        t: "anim",
        side: side === "ally" ? "foe" : "ally",
        kind: "faint",
      });
      ev.push({
        t: "text",
        s: `<b>${def.name}</b> collapses — its spirit returns to the amphora!`,
      });
      break;
    }
  }
