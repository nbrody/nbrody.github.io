...
// Procedural 7-segment digit SDF
float font2d_dist(vec2 tpos, float size, vec2 offset) {
    // The offset.x is the digit value (e.g. 2, 3, 4, 5...)
    // Our tpos is around 0. We map it to a box.
    float scl = 0.63 / size;
    vec2 p = tpos * scl;
    
    // Bounding box of the glyph (for the background/padding)
    vec2 box = abs(p) - 0.5;
    float bounding = max(box.x, box.y);
    
    // The digit to draw
    int n = int(offset.x + 0.5);
    if (n < 0 || n > 9) return bounding / scl;
    
    // Scale and shift for the 7-segment
    p = p * 2.5; 
    ...
    //  6
    vec2 points[6];
    points[0] = vec2(-0.5,  1.0);
    points[1] = vec2( 0.5,  1.0);
    points[2] = vec2(-0.5,  0.0);
    points[3] = vec2( 0.5,  0.0);
    points[4] = vec2(-0.5, -1.0);
    points[5] = vec2( 0.5, -1.0);
    
    float d = 1e5;
    
    // Bitmask for digits 0-9
    int[] segs = int[](
        0x77, 0x24, 0x5D, 0x6D, 0x2E, 0x6B, 0x7B, 0x25, 0x7F, 0x6F
    );
    int mask = segs[n];
    
    if ((mask & 1) != 0)   d = min(d, dseg(p, points[0], points[1])); // 0
    if ((mask & 2) != 0)   d = min(d, dseg(p, points[0], points[2])); // 1
    if ((mask & 4) != 0)   d = min(d, dseg(p, points[1], points[3])); // 2
    if ((mask & 8) != 0)   d = min(d, dseg(p, points[2], points[3])); // 3
    if ((mask & 16) != 0)  d = min(d, dseg(p, points[2], points[4])); // 4
    if ((mask & 32) != 0)  d = min(d, dseg(p, points[3], points[5])); // 5
    if ((mask & 64) != 0)  d = min(d, dseg(p, points[4], points[5])); // 6
    
    // The stroke width
    float df = d - 0.15;
    
    // Return max of bounding box and the digit SDF
    return max(df / 2.5, bounding) / scl;
}
